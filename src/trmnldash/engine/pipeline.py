"""Dashboard pipeline: walk a dashboard's layout, build each panel's
context, render, compose, quantize, save.
...
shape that flows through this function is:
    ...
    DashboardEntry
        -> [for each PanelSlot: panel.build_live_context() -> ctx,
...
`run_once` operates on a single `DashboardEntry`. The server-side
scheduler holds one of these per dashboard and calls `run_once` on
each one's own interval.
"""
...
from ..config import DashboardEntry
...
def run_once(entry: DashboardEntry, out_path: Path, *, quantize: bool = True) -> RenderStats:
    """Render one dashboard once and save. Returns per-stage timings.
    ...
    """
    stats = RenderStats(dashboard_name=entry.name, output_path=out_path)
    t_total = time.monotonic()

    device = entry.dashboard.device
